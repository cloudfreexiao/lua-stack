...
static unsigned long parse_eh_frame_int(void *content, size_t len, size_t *offset, dwarf_encoding enc) {
    ...
    default:
        LOG(ERROR, "Unhandled DWARF encoding type '0x%x'", enc & 0xf);
        return 0;
    }
}
...
static void parse_eh_frame_fde(dwarf_unwind_info_t *dinfo,
    ...
    dwarf_unwind_region_t *lastreg = VECTOR_GET_BACK_PTR(dwarf_unwind_region_t, &dinfo->regions);
    compute_one_region(dinfo, lastreg);
}
...
static void parse_eh_frame(dwarf_unwind_info_t *dinfo, unsigned long map_base,
    ...
    while(offset < len) {
        ...
        if(type == 0) {
            ...
            for(size_t i = 0; i < VECTOR_GET_SIZE(unsigned long, &cie_list.offset); i ++) {
                if(VECTOR_GET(unsigned long, &cie_list.offset, i) != cie_address) {
                    continue;
                }
    ...
    VECTOR_FOR_EACH_PTR(frame_cie_t, u, &cie_list.cie) {
		free(u->unwind_data);
	}

    VECTOR_FREE(unsigned long, &cie_list.offset);
    VECTOR_FREE(frame_cie_t, &cie_list.cie);
}
...
void load_dwarf_unwind_information(dwarf_unwind_info_t *dinfo) {
    elf_t *elf = &dinfo->item->elf;
    Elf64_Shdr *eh_frame = find_section_header_by_name(elf, ".eh_frame");
    if(eh_frame == NULL) {
        LOG(FATAL, "No eh_frame information found.");
    }

    void *eh_frame_content = (void *)((unsigned long)elf->map + eh_frame->sh_offset);
    unsigned long map_base = eh_frame->sh_addr - (unsigned long)eh_frame_content;
...
static void snapshot_computed_state(VECTOR_TYPE(precomputed_unwind_t) *unwinds,
    ...
    unsigned long cfa_ip, unsigned long next_ip,  map_item_t *item) {
    ...
    unsigned long orig_ip = unwind.ip;
    unwind.ip = orig_ip + item->addr_start - item->addr_offset;
    // printf("--> base: %lx, orig ip: %lx, ip: %lx, addr_start: %lx\n", region->base, orig_ip, unwind.ip, item->addr_start);
...
static void run_cfa(VECTOR_TYPE(precomputed_unwind_t) *unwinds,
    ...
    size_t cfa_length, int is_cie, map_item_t *item) {
    ...
    while(cursor < cfa_length && (ip >= cfa_ip || is_cie)) {
        if(ip < cfa_ip) {
            LOG(ERROR, "early CFA region found ip: %lx, cfa_ip: %lx\n", ip, cfa_ip);
            break;
        }
        ...
        case DW_CFA_same_value: {
            op1 = parse_eh_frame_uleb(cfa, cfa_length, &cursor);
            state->saved_registers[op1].from = REG_SAME;
            break;
        }
        ...
        default:
            LOG(ERROR, "Unsupported DWARF CFA opcode 0x%x", opcode);
            break;
        }
        count ++;
    }
    if(!is_cie) {
        ...
        dwarf_state_t *state = VECTOR_GET_BACK_PTR(dwarf_state_t, state_stack);
        snapshot_computed_state(unwinds, region, state, cfa_ip, cfa_ip+1, item);
    }
    else {
...
static void precompute_offsets_for(dwarf_unwind_info_t *dinfo,
    ...
    run_cfa(&dinfo->precomputed_unwinds, region, region->length+1,
        ...
        region->cie->unwind_data_length, 1, dinfo->item);

    VECTOR_PUSH(dwarf_state_t, &state_stack,
        *VECTOR_GET_PTR(dwarf_state_t, &state_stack, 0));

    run_cfa(&dinfo->precomputed_unwinds, region, region->length+1,
        &state_stack, region->unwind_data, region->unwind_data_length, 0, dinfo->item);

    VECTOR_FREE(dwarf_state_t, &state_stack);
}
...
void compute_one_region(dwarf_unwind_info_t *dinfo, dwarf_unwind_region_t *region) {
    long size_before = VECTOR_GET_SIZE(precomputed_unwind_t, &dinfo->precomputed_unwinds);
    precompute_offsets_for(dinfo, region);
    long size_after = VECTOR_GET_SIZE(precomputed_unwind_t, &dinfo->precomputed_unwinds);
